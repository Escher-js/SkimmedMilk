...
async function updateBranchList(folderPath) {
    const branches = await window.myAPI.updateBranchList(folderPath);
    ...
    // showCommitListをここで呼び出す
    console.log("-----commit log----")
    await showCommitList(folderPath);
    console.log("-------------------")
}
async function commitChanges(message) {
    const folderPath = folderPathSpan.textContent;
    window.myAPI.commitChanges(message, folderPath);
    window.myAPI.onCommitChanges((message) => console.log(message));
}
async function showCommitList() {
    ...
    window.myAPI.showCommitList(folderPath, selectedBranch)
        .then(commitLines => {
...
window.ipc.on('selected-folder', async (data) => {
    const { folderPath, isGitRepo } = data;
    ...
    if (!isGitRepo) {
        ...
        if (gitInitConfirmed) {
            console.log(`gitinit: Git repository initialized.`);
            window.git.initignore(folderPath);
            console.log(`checkedout to main: Switched to branch 'main'.`);
            window.myAPI.commitChanges('Initial commit', folderPath);
